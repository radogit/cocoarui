...
// Update the arrows on each tick
function ticked() {
    ...
    // Now add arrows for the forces
    forceArrows.selectAll(".force-arrow").remove();  // Clear previous arrows

    // Add new arrows based on the calculated forces
    nodeGroup.each(function(d) {
        ...
        if (Math.abs(dx) > 0 || Math.abs(dy) > 0) {
            ...
            const arrowLength = Math.min(length, 30);  // Limit arrow length to avoid overflow
            const angle = Math.atan2(dy, dx);

            d3.select(this).select(".force-arrows")  // Use the forceArrows group inside each node group
                .append("line")
                .attr("class", "force-arrow")
                .attr("x1", 0)  // Start at the center of the node
                .attr("y1", 0)
                .attr("x2", arrowLength * Math.cos(angle))
                .attr("y2", arrowLength * Math.sin(angle))
                .attr("stroke", "black")
                .attr("stroke-width", 2)
                .attr("marker-end", "url(#arrowhead)"); // Optional: add an arrowhead marker for the arrows
        }
    });
}
        ...
        nodes.forEach(d => {
            d.hotspots.forEach(hotspot => {
                ...
                if (distance > 1) {  // Avoid division by zero
                    const force = Math.exp(-distance / hotspot.width) * strength; // Gaussian decay
                    d.vx -= dx * force * alpha;
                    d.vy -= dy * force * alpha;
                }
            });
